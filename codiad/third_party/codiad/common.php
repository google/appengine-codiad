...
class Common {
  ...
  public static function formatJSEND($status,$data=false){
    ...
    if($status=="success"){
      ...
      $jsend = '{"status":"error","message":'.json_encode($data).$debug.'}';
    }
  ...
  //////////////////////////////////////////////////////////////////
  // Wrapper to 'file_get_contents' that bundles response code with returned response body
  //////////////////////////////////////////////////////////////////

  public static function FileGetContents($endpoint, $context_opts) {
    $body = file_get_contents($endpoint, false, stream_context_create($context_opts));

    // Extract a response code and return with the body.
    foreach ($http_response_header as $line) {
      if (preg_match('#HTTP/[0-9.]+\s+([0-9]+)#', trim($line), $matches)) {
        return array( 'code' => $matches[1], 'body' => $body );
      }
    }
    return array( 'code' => null, 'body' => $body );
  }

  //////////////////////////////////////////////////////////////////
  // Get metadata with the given URL
  //////////////////////////////////////////////////////////////////

  public static function GetMetadata($metadata_url) {
    $opts = array( 'http' => array( 'header' => "Metadata-Flavor: Google\r\n" ));
    $resp = Common::FileGetContents($metadata_url, $opts);
    if ($resp['code'] !== '200') {
      throw new Exception('Cannot retrieve information from Metadata server.', $resp['code']);
    }
    return $resp['body'];
  }

  //////////////////////////////////////////////////////////////////
  // Return the numeric id of the current Google Cloud project
  //////////////////////////////////////////////////////////////////

  public static function GetCloudProjectNum() {
    return Common::GetMetadata('http://metadata/computeMetadata/v1/project/numeric-project-id');
  }

  //////////////////////////////////////////////////////////////////
  // Return the id (name) of the current Google Cloud project
  //////////////////////////////////////////////////////////////////

  public static function GetCloudProjectId() {
    return Common::GetMetadata('http://metadata/computeMetadata/v1/project/project-id');
  }

  //////////////////////////////////////////////////////////////////
  // Return an access token or throws an exception if failed
  //////////////////////////////////////////////////////////////////

  public static function GetAccessToken() {
    $resp = Common::GetMetadata('http://metadata/computeMetadata/v1/instance/service-accounts/default/token');
    $parsed = json_decode($resp);
    if (!empty($parsed->access_token)) {
       return $parsed->access_token;
    }
    throw new Exception('Cannot retrieve an authentication token from this IDE instance.', $resp['code']);
  }

  //////////////////////////////////////////////////////////////////
  // Return the name of the current Google Source Repository
  //////////////////////////////////////////////////////////////////

  public static function GetCloudRepoName() {
    throw new Exception('Cannot list Google Source Repositories.');
  }

  //////////////////////////////////////////////////////////////////
  // Return the current Git revision id
  //////////////////////////////////////////////////////////////////

  public static function GetGitRevisionId() {
    $ret = Common::RunGitCommand('rev-parse HEAD');
    if ($ret->exit_code === 0 && !empty($ret->output[0])) {
      return $ret->output[0];
    }
    throw new Exception('Cannot get Git revision id.', $ret->exit_code);
  }

  public static function GetProjectRoot() {
    return WORKSPACE . '/' . $_SESSION['project'];
  }


  //////////////////////////////////////////////////////////////////
  // Run a git command in the current repo and returns the exit code and the output
  //////////////////////////////////////////////////////////////////
...
function runShellCommand($command) { return Common::runShellCommand($command); }
?>
