...
(function(global, $){
  ...
  codiad.filemanager = {
    ...
    init: function() {
      ...
      $('#finder-quick').click(function() {
        codiad.filemanager.search(codiad.project.getCurrent());
      });

      var _this = this;
      _this.updateBranch();
      setInterval(function() {_this.updateBranch();}, 10000);
    },

    updateBranch: function() {
      codiad.review.listBranchesNoError(function(response) {
        $.each(response.branches, function(i, b) {
          if (b === response.current_branch) {
            $('input[name="file-manager-branch-text"]').val(b);
            // Break out of each:
            return false;
          }
        });
    ...
    nodeListener: function() {
      ...
      $(document).on('selectstart', '#file-manager', false);

      $(document).on('click', '#file-manager span', function() { // Open or Expand
      ...
      $(document).on('click', '#file-manager a', function() { // Open or Expand
      ...
      }).on('contextmenu', '#file-manager a', function(e) { // Context Menu
        e.preventDefault();
    ...
    getExtension: function(path) {
      return this.getShortName(path).split('.').pop();
    },
          ...
          if (objectsResponse != 'error') {
            ...
            if (files.length > 0) {
              ...
              $.each(files, function(index) {
                ...
                appendage += '<li><span class="' + nodeClass + '"></span><a class="' +
                  files[index].type + ext + '" data-type="' + files[index].type +
                  '" data-path="' + files[index].name + '">' + name + '</a></li>';
              });
              appendage += '</ul>';
              if (rescan) {
                node.parent('li')
    ...
    openFile: function(path, focus, cursorOffset) {
      if (focus === undefined) {
        focus = true;
      }
      ...
      if ($.inArray(ext.toLowerCase(), this.noOpen) < 0) {
        ...
        $.get(this.controller + '?action=open&path=' + path, function(data) {
          ...
          if (openResponse != 'error') {
            node.removeClass('loading');
            codiad.active.open({'path': path, content: openResponse.content,
                               mtime: openResponse.mtime, sha1: openResponse.sha1},
                               focus, false /* readonly */, cursorOffset);
          }
        });
      } else {
      ...
      $.post(this.controller + '?action=modify&path='+path, data, function(resp){
        ...
        if (resp.status == 'success') {
          ...
          if (typeof callbacks.success === 'function'){
            context = callbacks.context || _this;
            callbacks.success.call(context, resp.data.mtime, resp.data.sha1);
          }
        } else {
          if (resp.message == 'Client is out of sync'){
            var reload = confirm(
              "Server has a more updated copy of the file. Would " +
              "you like to load the file from server? Press 'OK' to " +
              "load the server's copy. Press 'Cancel' to keep editing."
            );
            if (reload) {
              ...
              var buffer = codiad.active.activeBuffer;
              buffer.serverMTime = null;
              buffer.untainted = null;
            }
    ...
    createNode: function(path, type) {
      ...
      $(document).off('submit').on('submit', '#modal-content form', function(e) {
        ...
        $.get(codiad.filemanager.controller + '?action=create&path=' +
              createPath + '&type=' + type, function(data) {
          ...
          if (createResponse != 'error') {
            ...
            /* Notify listeners. */
            amplify.publish('filemanager.onCreate', {createPath: createPath, path: path,
      ...
      $(document).off('submit').on('submit', '#modal-content form', function(e) {
        ...
        $.get(_this.controller + '?action=delete&path=' + path, function(data) {
          ...
          if (deleteResponse != 'error') {
            var node = $('#file-manager a[data-path="' + path + '"]');
            var isDirectory = node.data('type') === 'directory';
            node.parent('li').remove();
            // Close any active files
            codiad.active.removeWithPath(path, isDirectory);
          }
          codiad.modal.unload();
        });
      });
      ...
      $(document).off('submit').on('submit', '#modal-content form', function(e) {
        ...
        $.post(_this.controller + '?action=search&path=' + path + '&type=' + searchType, {
          ...
          if (searchResponse != 'error') {
            $.each(searchResponse.results, function(key, val) {
              // Cleanup file format
              ...
              // Add result
              results += '<div><a onclick="codiad.filemanager.openFile(\'' + val.result +
                '\');setTimeout( function() { codiad.active.gotoLine(' + val.line +
                '); }, 500);codiad.modal.unload();">Line ' + val.line + ': ' +
                val.file + '</a></div>';
            });
    ...
    findFiles: function() {
      ...
      $(document).off('submit').on('submit', '#modal-content form', function(e) {
        ...
        $.post(_this.controller + "?action=find&path=" + codiad.project.getCurrent() + "&query=" + searchString, 
              {}, function(data) {
          response = JSON.parse(data);
          if (response.status != 'error') {
            var results = '';
            $.each(response.data.results, function(k, r) {
              // Add result
              results += '<div><a onclick="codiad.filemanager.openFile(\'' +
                r.file + '\');codiad.modal.unload();">' + r.display + '</a></div>';
            });
    ...
    /**
    * Retrieves the file status information from server and calls the 'callback' function
    * with the retrieved data.
    */
    fileInfo: function(path, callback) {
      $.get(this.controller + '?action=file_info&path=' + encodeURIComponent(path),
            function(data) {
        callback(data);
      });
    }
