...
class Filemanager extends Common {
      ...
      if(is_dir($this->path) && $handle = opendir($this->path)){
        ...
        foreach($index as $item=>$data){
          if($data['type']=='directory'){
            $folders[] = array("name"=>$data['name'],
                               "type"=>$data['type'],
                               "size"=>$data['size']);
          }
          if($data['type']=='file'){
            $files[] = array("name"=>$data['name'],
                             "type"=>$data['type'],
                             "size"=>$data['size']);
          }
        }
  ...
  public function find() {
    if (!function_exists('shell_exec')) {
      ...
      $input = escapeshellarg("*{$this->query}*");
      $cmd = 'find ' . $this->path;
      $cmd = "$cmd -not -path '*/\.git*' -ipath $input";
      $cmd = "$cmd  -printf \"%y:%h/%f\n\"";
      ...
      foreach ($output_arr as $line) {
        ...
        $da = array();
        if ($data[0] == 'f') {
          $da['display'] = str_replace($this->path,'',$data[1]);
          $da['file'] = str_replace($this->root,'',$data[1]);
          $return[] = $da;
        }
      }
      if (count($return) == 0) {
  ...
  public function search(){
    if(!function_exists('shell_exec')){
      ...
      $output = shell_exec('find -L ' . $this->path . ' -not -path "*/\.git/*" -iregex  ".*' .
                           $this->search_file_type  . '" ' .
                           " -type f | xargs --no-run-if-empty fgrep -i -I -n -R -H -o $input");
      $output_arr = explode("\n", $output);
      $return = array();
      foreach($output_arr as $line){
        $data = explode(":", $line);
  ...
  public function open(){
    if(is_file($this->path)){
      ...
      $this->data->sha1 = sha1_file($this->path);
    }else{
      $this->status = "error";
  ...
  /**
  * Returns the file information in a json. The info includes:
  * - File's absolute path on server
  * - Existence
  * - Modification time
  * - Does file belong to the current project?
  */
  public function FileInfo() {
    $response = new stdClass;
    $response->path = $this->path;
    $response->exists = file_exists($this->path);
    if ($response->exists) {
      $response->mtime = filemtime($this->path);
      $response->sha1 = sha1_file($this->path);
    } else {
      $response->mtime = -1;
      $response->sha1 = "";
    }
    if (strpos($response->path, $this->root . $this->project) === 0) {
      $response->in_current_project = true;
    } else {
      $response->in_current_project = false;
    }
    $this->status = 'success';
    $this->data->file_info = $response;
    $this->respond();
  }
  ...
  public function respond(){
    ...
    if ($this->status=="success") {
      if ($this->data) {
        $json = '{"status":"success","data":'.json_encode($this->data).'}';
      }else{
