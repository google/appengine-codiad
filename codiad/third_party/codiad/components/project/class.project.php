...
class Project extends Common {
  ...
  public function Create(){
    if($this->name != '' && $this->path != '') {
      ...
      if($this->isAbsPath($this->path)) {
        echo formatJSEND('error', 'Path cannot be absolute.');
        return;
      }
      if($this->path != '') {
        $pass = $this->CheckDuplicate();
        if($pass){
          mkdir(WORKSPACE . '/' . $this->path, 0755, true);
          ...
          if($this->gitrepo && filter_var($this->gitrepo, FILTER_VALIDATE_URL) !== false){
            ...
            $this->command_exec = "cd " . escapeshellarg(WORKSPACE . '/' . $this->path);
            $this->command_exec .= " && git init "
              . " && git remote add origin " . escapeshellarg($this->gitrepo)
              . " && git pull origin " . escapeshellarg($this->gitbranch)
              . " && git checkout " . escapeshellarg($this->gitbranch);
  ...
  public function SanitizeGitBranch(){
    $sanitized = str_replace(
      array("..",chr(40), chr(177),"~","^",":","?","*","[","@{","\\"),
      array(""), $this->gitbranch);
    return $sanitized;
  }
  ...
  public function Rename(){
    ...
    $revised_array[] = $this->projects[] =
      array("name"=>$_GET['project_name'],"path"=>$this->path);
